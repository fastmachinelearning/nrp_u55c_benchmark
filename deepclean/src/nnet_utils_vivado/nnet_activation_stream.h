...
template<class data_T, class res_T, typename CONFIG_T>
void tanh_ss(hls::stream<data_T> &data, hls::stream<res_T> &res) {
    // Initialize the lookup table
#ifdef __HLS_SYN__
    bool initialized = false;
    typename CONFIG_T::table_t tanh_table[CONFIG_T::table_size];
    ...
    TanHActLoop: for (int i = 0; i < CONFIG_T::n_in; i++) {
        #pragma HLS PIPELINE

        data_T in_data = data.read();
        res_T out_data;

        int data_round = in_data*CONFIG_T::table_size/8;
        int index = data_round + 4*CONFIG_T::table_size/8;
        if (index < 0)   index = 0;
        else if (index > CONFIG_T::table_size-1) index = CONFIG_T::table_size-1;
        out_data = tanh_table[index];
        res.write(out_data);
    }
}
